/**
 * One-time (idempotent) import of the real pricing/fabric/colour/size/
 * settings data from the CSV exports sitting in the repo root:
 ...
 *   Max_Size_Master.csv, Settings.csv
 * (Customer_Master.csv has no data rows — customers start empty per the brief.)
 *
 * Run with: npm run import:real
 *
 * Wipes and reloads all category/fabric/colour/band/price/discount/max-size
 * data (not suppliers, customers, or quotes) so it's safe to re-run whenever
 * an updated CSV drop arrives.
 */
import fs from "fs";
...
async function chunkedCreateMany<T>(model: { createMany: (args: { data: T[] }) => Promise<unknown> }, rows: T[], size = 5000) {
...
async function main() {
  console.log("Wiping existing category/fabric/colour/band/price/discount/max-size data...");
  // ProductCategory cascades to Fabric, FabricBandMapping, PriceListRow,
  // MaxSizeRule, and Colour. DiscountRate needs an explicit delete too since
  // its supplier-level default fallback rows (categoryId = null) don't
  // cascade from a category delete.
  ...
  // 1. Categories
  // -------------------------------------------------------------------
  console.log("Importing Product_Category_Master.csv...");
  ...
  console.log(`  ${categoryCache.size} categories.`);

  // -------------------------------------------------------------------
  // 2. Decora Fabrics + Band Mappings
  // -------------------------------------------------------------------
  console.log("Importing Fabric_and_Item_Band_Master.csv...");
  ...
  console.log(`  ${bandMappingsCreated} band mappings created, ${bandMappingsSkipped} rows skipped (missing required fields).`);

  // -------------------------------------------------------------------
  // 3. Decora Price List Rows (large: ~34k rows)
  // -------------------------------------------------------------------
  console.log("Importing Price_Table_Master.csv (this is the big one)...");
  ...
  console.log(`  ${priceCreated} price rows created, ${priceSkipped} rows skipped (missing/invalid fields).`);

  // -------------------------------------------------------------------
  // 4. Colours (informational only, scoped to category)
  // -------------------------------------------------------------------
  console.log("Importing Colour_Master.csv...");
  ...
  console.log(`  ${coloursCreated} distinct colours created.`);

  // -------------------------------------------------------------------
  // 5. Beverley Fabric Bands (Roller + Vertical only; no price grid yet)
  // -------------------------------------------------------------------
  console.log("Importing Beverley_Fabric_Bands.csv...");
  ...
  console.log(`  ${beverleyCreated} Beverley band mappings created, ${beverleySkipped} rows skipped.`);

  // -------------------------------------------------------------------
  // 6. Max Size Rules
  // -------------------------------------------------------------------
  console.log("Importing Max_Size_Master.csv...");
  ...
  console.log(`  ${maxSizeCreated} max-size rules created, ${maxSizeSkipped} rows skipped (notes/gaps, not real rules).`);
  ...
  // -------------------------------------------------------------------
  console.log("Importing Settings.csv...");
  const settingsContent = fs.readFileSync(path.join(ROOT, "Settings.csv"), "utf-8");
  ...
  console.log(
    `  VAT=${vatRate ? Number(vatRate) * 100 : "?"}%, deposit default=${depositPercent ? Number(depositPercent) * 100 : "?"}%, ` +
      `Premium×${premiumMultiplier ?? "?"}, Negotiation×${negotiationMultiplier ?? "?"}`
  );
  ...
  console.log(`  ${overridesCreated} category discount overrides created, ${overridesSkipped} unmapped rows skipped.`);

  console.log("\nImport complete.");
  console.log(`Categories: ${categoryCache.size}, Fabrics: ${fabricCache.size}`);
}
